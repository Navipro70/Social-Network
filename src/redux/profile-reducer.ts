...
export const actions = {
  ...
  setProfileStatus: (statusText: string) =>
    ({ type: 'profile/SET_PROFILE_STATUS', statusText } as const),
  getProfileStatus: (statusText: string) =>
    ({ type: 'profile/GET_PROFILE_STATUS', statusText } as const),
  toggleProfileFetching: (isProfileFetching: boolean) =>
    ({
      type: 'profile/FETCHING_PROFILE',
      isProfileFetching,
    } as const),
  setProfilePage: (profile: profileType) =>
    ({ type: 'profile/SET_PROFILE_PAGE', profile } as const),
  uploadProfilePhoto: (photoFiles: photosType) =>
    ({ type: 'profile/UPLOAD_PROFILE_PHOTO', photoFiles } as const),
}
...
export type ThunkPutUserInfoType = ThunkAction<
  Promise<void>,
  AppStateType,
  unknown,
...
export const thunkPutUserInformation = (
  data: ProfileInformationType,
  userId: number,
): ThunkPutUserInfoType => async (dispatch) => {
  let responseStatusInfo = await profileApi.setInformation(data)
  if (responseStatusInfo.resultCode === ResultCodesEnum.Success)
    await dispatch(thunkSetCurrentProfile(userId))
...
export const thunkSavePhoto = (photoFile: photosType): ThunkActionType => async (dispatch) => {
  ...
  if (data.resultCode === ResultCodesEnum.Success)
    dispatch(actions.uploadProfilePhoto(data.data.photos))
}
